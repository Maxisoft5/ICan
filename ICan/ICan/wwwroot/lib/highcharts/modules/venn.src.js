...
(function (factory) {
    ...
    _registerModule(_modules, 'Series/VennSeries.js', [_modules['Core/Animation/AnimationUtilities.js'], _modules['Core/Series/Series.js'], _modules['Core/Color/Color.js'], _modules['Mixins/DrawPoint.js'], _modules['Mixins/Geometry.js'], _modules['Mixins/GeometryCircles.js'], _modules['Mixins/NelderMead.js'], _modules['Core/Utilities.js']], function (A, BaseSeries, Color, DrawPointMixin, GeometryMixin, GeometryCirclesModule, NelderMeadMixin, U) {
         ...
         * The margin describes if the point is well placed within the public circles,
         * and away from the external
         * @private
         * @todo add unit tests.
         * @param {Highcharts.PositionObject} point
         * The point to evaluate.
         * @param {Array<Highcharts.CircleObject>} public
         * The public circles.
        ...
        var getMarginFromCircles = function getMarginFromCircles(point,
            public,
            external) {
                var margin = public.reduce(function (margin,
            circle) {
         ...
         * distance from the public circles, and as large possible distane to the
         * external circles.
         * @private
         ...
         */
        var getLabelPosition = function getLabelPosition(public,
            external) {
                // Get the best label position within the public circles.
                var best = public.reduce(function (best,
            circle) {
                    var d = circle.r / 2;
            ...
            if (!(isPointInsideAllCircles(best, public) &&
                isPointOutsideAllCircles(best, external))) {
                // If point was either outside one of the public, or inside one of the
                // external, then it was invalid and should use a fallback.
                if (public.length > 1) {
                    best = getCenterOfPoints(getCirclesIntersectionPolygon(public));
                }
                else {
                    best = {
                        x: public[0].x,
         ...
         * finding the largest distance, which is inside all public circles, and
        ...
        var getLabelWidth = function getLabelWidth(pos,
            public,
            external) {
                var radius = public.reduce(function (min,
            circle) {
        ...
        function getLabelValues(relation, setRelations) {
            ...
            // Create a list of public and external circles.
            var data = setRelations.reduce(function (data,
                set) {
                    // If the set exists in this relation, then it is public,
                    // otherwise it will be external.
                    var isInternal = sets.indexOf(set.sets[0]) > -1;
                var property = isInternal ? 'public' : 'external';
