...
{
	public class UpdReport : Report
	{
		private int _lastRow = -1;

		protected override string DateFormat => "d MMMM yyyy г.";
		protected override string DateCell => "Z1";
		protected override string ReportNumCell => "Q1";

		protected override string BookIdColumn => "D";


		protected override string BookAmountColumn => "AA";
		protected override string TotalSumColumn => "BE";

		protected override string BookTotalSumColumn => "BE";

		public override bool CanSkipRowItems => true;


		public UpdReport(ExcelWorksheet worksheet, DataSet data, int shopId, ReportKind reportKind, IEnumerable<OptReportCriteriaGroup> criteriaGroups = null,
			...
			{
				{
					if (_firstRow == -1)
					{
						...
						{
							var cellValue = _worksheet.Cells[i, 2].Value?.ToString().ToLower().Trim();
							...
							cellValue = _worksheet.Cells[i, 6].Value?.ToString().ToLower().Trim();
							if (!string.IsNullOrWhiteSpace(cellValue)
									&& cellValue.Contains("п/п"))
			...
			get
			{
				if (_lastRow == -1)
				{
					...
					for (var i = 1; i <= maxRow; i++)
					{
						...
						cellValue = _worksheet.Cells[i, 7].Value?.ToString();
						if (!string.IsNullOrWhiteSpace(cellValue)
									&& cellValue.ToLower().Trim().Contains("всего к оплате"))
						{
							_lastRow = i - 1;
